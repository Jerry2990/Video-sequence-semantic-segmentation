...
# from util.general import load_model
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, norm_layer=nn.BatchNorm2d, bn_eps=1e-5,
                 bn_momentum=0.1, deep_stem=False, stem_width=32, inplace=True, in_planes = 3):
        self.inplanes = stem_width * 2 if deep_stem else 64
        super(ResNet, self).__init__()
        if deep_stem:
            self.conv1 = nn.Sequential(
                nn.Conv2d(in_planes, stem_width, kernel_size=3, stride=2, padding=1,
                          bias=False),
            ...
            self.conv1 = nn.Conv2d(in_planes, 64, kernel_size=7, stride=2, padding=3,
                                   bias=False)
